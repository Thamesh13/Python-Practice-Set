...
student_data_dict = student_data.json()  # This is now a dictionary

# Access the 'Student Data' key
return render_template('edit_form.html', student=student_data_dict['Student Data'])

    student_data_dict = student_data.json()  # Ensure this is only called once
    return render_template('edit_form.html', student=student_data_dict['Student Data'])
...
def add_data():
    if request.method=="POST":
       ...
       response=requests.post("http://127.0.0.1:5000/students",json=student_data)
...
def edit_student(id):
    if request.method == 'GET':
        student_data = calling_student.get_Student_data_By_id(id)
        if student_data.status_code == 200:
            return render_template('edit_form.html', student=student_data.json()['Student Data'])
        else:
