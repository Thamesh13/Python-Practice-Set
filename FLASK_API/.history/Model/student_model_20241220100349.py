...
class Student:
    ...
    def post_student_data(self):
        data=request.json
        try:
            quary.execute('insert into student(Student_Name,Student_Age,Student_Contact_Number,Student_Contact_email) values(?,?,?,?)',tuple(data.values()))
    ...
    def update_student_data (self,id):
        ...
        print(data)
        try:
            quary.execute("Update Student Set Student_Name=? ,Student_Age =?,Student_Contact_Number =?,Student_Contact_Email=? where Id=?",tuple(data))
    ...
    def get_Student_data_By_id (self,id):
        ...
        try:
            ...
            return make_response({"success" : "data retrived successfully!!!","Student Data":student_data})
        except Exception as e:
            return make_response({"error" : f"Something error occured as {e}"},400)